...
import { Plus, Search, Pencil, Trash2, Building2, Loader2, X, Check } from 'lucide-react'
...
const EMPTY_FORM = { name: '', category: '', gst_number: '', email: '', phone_number: '', status: 'Pending' }
...
export default function VendorsPage() {
  ...
  const openEdit = (v: Vendor) => {
    setEditing(v)
    setForm({ name: v.name, category: v.category || '', gst_number: v.gst_number || '', email: v.email || '', phone_number: v.phone_number || '', status: v.status })
    setShowModal(true)
  }
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage your approved supplier network</p>
        </div>
        <button onClick={openCreate} className="flex items-center gap-2 btn-primary w-auto px-5 py-2.5">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-surface-100 border-b border-gray-200">
                <tr>
                  {['Name', 'Category', 'Email', 'Phone', 'GST Number', 'Status', 'Actions'].map(h => (
                  ...
                  <tr key={v.id} className="hover:bg-surface-100 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(v)} className="p-1.5 rounded-lg hover:bg-brand-50 text-gray-400 hover:text-brand-600 transition-colors">
                          <Pencil size={15} />
                        </button>
                        <button onClick={() => handleDelete(v.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600 transition-colors">
                          <Trash2 size={15} />
                        </button>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-slide-up">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-gray-900">{editing ? 'Edit Vendor' : 'Add New Vendor'}</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input className="input-field" placeholder="e.g. Acme Supplies" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} />
              </div>
              ...
              <div>
                ...
                <input className="input-field" type="email" placeholder="vendor@company.com" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="form-label">Phone</label>
                  <input className="input-field" placeholder="+91 …" value={form.phone_number} onChange={e => setForm(p => ({ ...p, phone_number: e.target.value }))} />
                </div>
                <div>
                  <label className="form-label">GST Number</label>
                  <input className="input-field" placeholder="22AAAAA0000A1Z5" value={form.gst_number} onChange={e => setForm(p => ({ ...p, gst_number: e.target.value }))} />
                </div>
              </div>
            </div>
